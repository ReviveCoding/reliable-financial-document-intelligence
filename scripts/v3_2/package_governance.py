...
for path in evidence_paths:
    manifest.append({"path": str(path.relative_to(ROOT)), "sha256": sha(path), "bytes": path.stat().st_size, "protocol_sha256": protocol_sha})
...
registry = [
    ...
    ["V3.2-E01", "synthetic_line_item_evaluator", "PASS_10_CASES", "DEVELOPMENT", "tests/test_v3_2_line_item_evaluation.py"],
    ["V3.2-E02", "row_aware_line_item_evaluation", "COMPLETE", "RETROSPECTIVE_LOCKED_BENCHMARK", "artifacts/v3_2/line_items/retrospective_test/aggregate_metrics.json"],
    ["V3.2-E03", "production_safe_feature_table", "COMPLETE_NO_LEAKAGE", "DEVELOPMENT_AND_RETROSPECTIVE", "artifacts/v3_2/data/risk_feature_table.csv"],
    ["V3.2-E04", "grouped_cv_risk_model_selection", "R4_BEST_LEARNED_R0_BEST_OVERALL", "DEVELOPMENT", "artifacts/v3_2/risk_model/development_selection/frozen_selection.json"],
    ["V3.2-E05", "finite_sample_certification", "NO_TARGET_CERTIFIED", "INDEPENDENT_CERTIFICATION", "artifacts/v3_2/risk_control/certification_results.csv"],
    ...
    ["V3.2-E07", "risk_ranker_robustness", "COMPLETE_REUSED_V3_1", "DEVELOPMENT_PROXY", "artifacts/v3_2/robustness/robustness_risk_response_summary.csv"],
...
state = {
    ...
    "selection_freeze_commit": "3532686",
...
- **Fresh external LIR validation:** still missing. Promotion should not be reconsidered without a larger grouped development/certification sample and authorized external line-item evidence.
- No mandatory implementation blocker remains.
""")
print(json.dumps({"decision": state["decision"], "manifest_entries": len(manifest), "protocol_sha256": protocol_sha}, sort_keys=True))
