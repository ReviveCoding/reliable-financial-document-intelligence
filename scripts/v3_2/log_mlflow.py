...
with mlflow.start_run(run_id=run_id, run_name=None if run_id else "v3.2-methodology-correction-replay") as run:
    mlflow.log_param("analysis_version", "3.2.0")
    mlflow.log_param("protocol_sha256", hashlib.sha256(protocol.read_bytes()).hexdigest())
    mlflow.log_param("methodology_erratum_sha256", hashlib.sha256((ROOT / "configs/v3_2/methodology_erratum.json").read_bytes()).hexdigest())
    ...
    mlflow.log_param("fresh_confirmatory_evidence", False)
    mlflow.log_param("replay_designation", "METHODOLOGY_CORRECTION_REPLAY")
